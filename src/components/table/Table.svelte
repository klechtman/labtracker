...
<div class="flex flex-col justify-end h-full" role="presentation" on:click={handleTableClick}>
  {#if rowNumbersOnly}
    <div class="grid flex-grow w-full gap-px h-full" style="grid-template-rows: repeat({maxRows}, 1fr); grid-template-columns: 1fr; margin-left: 0;">
  ...
  {:else}
    <div class="grid flex-grow w-full gap-px relative min-w-0 h-full" data-columns={columns} style="grid-template-rows: repeat({maxRows}, 1fr); grid-template-columns: repeat({columns}, minmax(60px, 1fr)); margin-left: 0; min-width: {columns * 60}px;">
