<script>
  ...
  function handleRenameMouseenter() { renameHover = true; }
  function handleRenameMouseleave() { renameHover = false; }
  ...
  $: console.log('DEBUG X:', { value, disabled, isLinkMode });
</script>
...
<div class="relative group-dropdown">
  {#if isRenaming}
    <div class={getInputClasses()}>
      <span
        ...
        style="background-color: {getGroupColor(groups[value]?.color)};"
      ></span>
      <input
  ...
  {:else}
    <div
      role="button"
      tabindex="0"
      class={getInputClasses() + (renameHover ? ' rename-hover' : '')}
    ...
    >
      <span
        class="w-5 h-5 rounded border border-slate-300 flex-shrink-0"
        style="background-color: {value ? getGroupColor(groups[value]?.color) : '#cbd5e1'};"
      ></span>
      <span class="flex-1 min-w-0 text-left">
        {#if value && groups[value]}
          <span
            ...
            on:click|stopPropagation={handleRenameAreaClick}
